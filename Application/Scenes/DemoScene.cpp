...
public:
	...
	struct FSQ : PipelinePass {

		FrameBufferAttachment depth;
		Mesh<TexVertex> mesh;
	} fsq;

	struct DebugLineList : PipelinePass {
		FrameBufferAttachment depth;
		Mesh<PosVertex> mesh;
	...
	struct DebugLineStrip : PipelinePass {
	...
	void Create(std::weak_ptr<Window> window, bool enabledOverlay) override
	{
		RenderingContext::Create(window, enabledOverlay);
		this->window = window;
		...
		for (int i = 0; i < 1000; ++i) {
			...
			transform1.SetPosition(glm::vec3(utils::Random(-10.0f, 10.0f),
											 utils::Random(-10.0f, 10.0f),
											 utils::Random(-10.0f, 10.0f)));
			//transform1.SetPosition({ -1.0f, -1.0f, -1.0f });
			//const std::string room = std::string(ASSET_DIR) + "Models/teapot.obj";
			transform1.SetRotation(glm::vec3(utils::Random(0.0f, 360.0f),
											 utils::Random(0.0f, 360.0f),
											 utils::Random(0.0f, 360.0f)));
...
//		RenderingContext::Destroy();
	...
	void InitializeAttachments()
	{
		...
		// ----------------------
		{
			...
			gBuffer.renderPass.Create(rpInfo, vk::Extent2D(FB_SIZE.x, FB_SIZE.y), clearValues, &device);
		}
		// Frame Buffers
		{
			std::array<vk::ImageView, 4> attachments;
			attachments[0] = gBuffer.position.imageView.VkType();
			attachments[1] = gBuffer.normal.imageView.VkType();
			attachments[2] = gBuffer.albedo.imageView.VkType();
			attachments[3] = gBuffer.depth.imageView.VkType();

			vk::FramebufferCreateInfo fbInfo = {};
			fbInfo.renderPass = gBuffer.renderPass.VkType();
		...
		// Command Buffers
		{
			...
			cmdInfo.commandPool = commandPool.VkType();
		...
		// ----------------------
		{
			// Render pass & subpass
			{
				...
				fsq.renderPass.Create(createInfo, swapchain.extent, clearValues, &device);
			}
			...
			// Frame buffers
			{
				...
				// FB layers
				createInfo.layers = 1;
				createInfo.renderPass = fsq.renderPass.VkType();
		...
		// ----------------------
		{
			// Render pass & subpass
			{
				...
				debugLineList.renderPass.Create(createInfo, extent, clearValues, &device);
			}
			...
			// Frame buffers
			{
				...
				// FB layers
				createInfo.layers = 1;
				createInfo.renderPass = debugLineList.renderPass.VkType();
		...
		// ----------------------
		{
			// Render pass & subpass
			{
				...
				debugLineStrip.renderPass.Create(createInfo, extent, clearValues, &device);
			}
			...
			// Frame buffers
			{
				...
				// FB layers
				createInfo.layers = 1;
				createInfo.renderPass = debugLineStrip.renderPass.VkType();
				createInfo.attachmentCount = 2;
	...
	void LoadSection(int section = -1)
	{
		...
		if (section != -1) {
			const std::string sectionString =
					std::string(ASSET_DIR) + "Models/Section" + std::to_string(section + 1) + ".txt";
	...
	void InitializeDescriptorSets()
	{
		...
		std::array<WriteDescriptorSet, 5> descriptorInfos =
				{
						WriteDescriptorSet::CreateAsync(0,
														vk::ShaderStageFlagBits::eVertex,
														vk::DescriptorType::eUniformBuffer,
														vpDescInfos),
						WriteDescriptorSet::Create(1,
												   vk::ShaderStageFlagBits::eFragment,
												   vk::DescriptorType::eCombinedImageSampler,
												   posDesc),
						WriteDescriptorSet::Create(2,
												   vk::ShaderStageFlagBits::eFragment,
												   vk::DescriptorType::eCombinedImageSampler,
												   normalDesc),
						WriteDescriptorSet::Create(3,
												   vk::ShaderStageFlagBits::eFragment,
												   vk::DescriptorType::eCombinedImageSampler,
												   albedoDesc),
						WriteDescriptorSet::CreateAsync(4,
														vk::ShaderStageFlagBits::eFragment,
														vk::DescriptorType::eUniformBuffer,
	...
	void InitializePipelines()
	{
		...
		pipelineInfo.layout = (gBuffer.pipelineLayout).VkType();                            // Pipeline Layout pipeline should use
		pipelineInfo.renderPass = gBuffer.renderPass.VkType();                            // Render pass description the pipeline is compatible with
		pipelineInfo.subpass = 0;                                        // Subpass of render pass to use with pipeline

		gBuffer.pipeline.Create(pipelineInfo, &device);

		// ----------------------
		// Deferred wireframe
		// ----------------------
		rasterizeState.setPolygonMode(vk::PolygonMode::eLine);
		gBuffer.wireframePipeline.Create(pipelineInfo, &device);

		rasterizeState.setPolygonMode(vk::PolygonMode::eFill);
		// ----------------------
		// FSQ PIPELINE
		// ----------------------
		vertInfo = vertModule.Load(
				"fsqVert.spv",
				vk::ShaderStageFlagBits::eVertex,
				&device
		);
		fragInfo = fragModule.Load(
				"fsqFrag.spv",
				vk::ShaderStageFlagBits::eFragment,
				&device
		);
		...
		pipelineInfo.renderPass = fsq.renderPass.VkType();

		fsq.pipelineLayout.Create(layout, &device);
		pipelineInfo.layout = fsq.pipelineLayout.VkType();
		...
		colorBlendAttachments.colorWriteMask = vk::ColorComponentFlags(
				VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
				VK_COLOR_COMPONENT_A_BIT);
		...
		fsq.pipeline.Create(pipelineInfo, &device);
		...
		pipelineInfo.pStages = shaderStages;
		pipelineInfo.renderPass = renderPass.VkType();

		pipelineLayout.Create(layout, &device);
		pipelineInfo.layout = pipelineLayout.VkType();
		...
		pipeline.Create(pipelineInfo, &device);
		...
		pipelineInfo.pStages = shaderStages;
		pipelineInfo.renderPass = debugLineList.renderPass.VkType();
		...
		pipelineInfo.layout = debugLineList.pipelineLayout.VkType();

		debugLineList.pipeline.Create(pipelineInfo, &device);
		...
		pipelineInfo.pStages = shaderStages;
		pipelineInfo.renderPass = debugLineStrip.renderPass.VkType();
		inputAssembly.topology = vk::PrimitiveTopology::eTriangleList;
		...
		pipelineInfo.layout = debugLineStrip.pipelineLayout.VkType();

		debugLineStrip.pipeline.Create(pipelineInfo, &device);
	}
	...
	void RecordDeferred(uint32_t imageIndex)
	{
		...
		gBuffer.renderPass.Begin(
				cmdBuf,
				gBuffer.frameBuffers[imageIndex].VkType(),
				(gBuffer.wireframeEnabled) ? gBuffer.wireframePipeline.VkType() : gBuffer.pipeline.VkType(),
				//vk::SubpassContents::eSecondaryCommandBuffers
				vk::SubpassContents::eInline
		);

		vk::CommandBufferInheritanceInfo inherit = {};
		inherit.setFramebuffer(gBuffer.frameBuffers[imageIndex].VkType());
		inherit.setRenderPass(gBuffer.renderPass.VkType());

		if (gBuffer.render) {
			renderSystem->RenderEntities<DeferredRenderComponent>(
					cmdBuf,
					descriptors.sets[imageIndex],
					gBuffer.pipelineLayout.VkType()
			);
	...
	void RecordFSQ(uint32_t imageIndex)
	{
		...
		fsq.renderPass.Begin(
				cmdBuf,
				fsq.frameBuffers[imageIndex].VkType(),
				fsq.pipeline.VkType()
		);
	...
	void RecordDepthCopyForward(uint32_t imageIndex)
	{
		...
		if (copyDepth) {
			...
			cmdBuf.copyImage(
					gBuffer.depth.image.VkType(), vk::ImageLayout::eTransferSrcOptimal,
					depth.image.VkType(), vk::ImageLayout::eTransferDstOptimal,
					imageCopy);
		}
	...
	void RecordForward(uint32_t imageIndex)
	{
		...
		renderPass.Begin(
				cmdBuf,
				frameBuffers[imageIndex].VkType(),
				pipeline
		);
	...
	void RecordDepthCopyDebug(uint32_t imageIndex)
	{
		...
		cmdBuf.clearDepthStencilImage(
				debugLineList.depth.image.VkType(),
				vk::ImageLayout::eTransferDstOptimal,
				vk::ClearDepthStencilValue(1.0f),
				range
		);
		...
		if (copyDepth) {
			...
			cmdBuf.copyImage(
					depth.image.VkType(), vk::ImageLayout::eTransferSrcOptimal,
					debugLineList.depth.image.VkType(), vk::ImageLayout::eTransferDstOptimal,
					imageCopy);
		}
	...
	void RecordDebugLineList(uint32_t imageIndex)
	{
		...
		debugLineList.renderPass.Begin(
				cmdBuf,
				debugLineList.frameBuffers[imageIndex].VkType(),
	...
	void RecordDebugLineStrip(uint32_t imageIndex)
	{
		...
		debugLineStrip.renderPass.Begin(
				cmdBuf,
				debugLineStrip.frameBuffers[imageIndex].VkType(),
...
//			}
	...
	void OnSurfaceRecreate()
	{
		...
		uboViewProjection.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 3.0f),
	...
	void UpdateObjects(float dt)
	{
		...
		uboComposition.lights[1].position.x =
				-4.0f + sin(glm::radians(360.0f * timer * lightMovementMod) + 45.0f) * 2.0f;
		uboComposition.lights[1].position.z =
				0.0f + cos(glm::radians(360.0f * timer * lightMovementMod) + 45.0f) * 2.0f;
		...
		uboComposition.lights[4].position.x =
				0.0f + sin(glm::radians(360.0f * timer * lightMovementMod + 90.0f)) * 5.0f;
		uboComposition.lights[4].position.z =
				0.0f - cos(glm::radians(360.0f * timer * lightMovementMod + 45.0f)) * 5.0f;

		uboComposition.lights[5].position.x =
				0.0f + sin(glm::radians(-360.0f * timer * lightMovementMod + 135.0f)) * 10.0f;
		uboComposition.lights[5].position.z =
				0.0f - cos(glm::radians(-360.0f * timer * lightMovementMod - 45.0f)) * 10.0f;
...
//				{
//					octree.Create(position, halfExtent, depth);
//				}
//			}
//			else
//			{
...
bk::RenderingContext& bk::RenderingContext::Get()
{
